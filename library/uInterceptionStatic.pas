...
function interception_get_precedence(context: InterceptionContext; device: InterceptionDevice): InterceptionPrecedence;
procedure interception_set_precedence(context: InterceptionContext; device: InterceptionDevice; precedence: InterceptionPrecedence);
function interception_get_filter(context: InterceptionContext; device: InterceptionDevice): InterceptionFilter;
procedure interception_set_filter(context: InterceptionContext; interception_predicate: InterceptionPredicate; filter: InterceptionFilter);
function interception_wait(context: InterceptionContext): InterceptionDevice;
function interception_wait_with_timeout(context: InterceptionContext; milliseconds: NativeUInt): InterceptionDevice;
function interception_send(context: InterceptionContext; device: InterceptionDevice; const stroke: PInterceptionStroke; nstroke: Cardinal): Integer;
function interception_receive(context: InterceptionContext; device: InterceptionDevice; stroke: PInterceptionStroke; nstroke: Cardinal): Integer;
function interception_get_hardware_id(context: InterceptionContext; device: InterceptionDevice; hardware_id_buffer: Pointer; buffer_size: SizeInt): Cardinal;
...
const interception_context_size = INTERCEPTION_MAX_DEVICE * sizeof (_InterceptionDeviceArray);

function interception_create_context: InterceptionContext;
// see https://github.com/oblitum/Interception/blob/513556e660893ca294b2e287143abb40a9170bb3/library/interception.c#L43
var device_array : InterceptionDeviceArray;
//                                         012345678901234567  8
var device_name : array [0..18] of char = '\\.\interception00'#0;
var device_name_num : PChar;
var bytes_returned : cardinal;
var i : InterceptionDevice;

var  zero_padded_handle : array [0..1] of HANDLE;
...
begin
    ...
    if not assigned (device_array) then exit;
    ...
    begin
        ...
        device_array[i].handle := CreateFileA(device_name, GENERIC_READ, 0, nil, OPEN_EXISTING, 0, 0);
        ...
        begin
         ...
         if(not DeviceIoControl(device_array[i].handle, IOCTL_SET_EVENT, @zero_padded_handle[0], sizeof(zero_padded_handle), nil, 0, @bytes_returned, nil)) then
         begin
            interception_destroy_context(result);
            result := nil;
            exit;
...
begin
    ...
    begin
       ...
       DeviceIoControl(device_array[device - 1].handle, IOCTL_GET_PRECEDENCE, nil, 0, @result, sizeof(InterceptionPrecedence), @bytes_returned, nil);
    end;
...
begin
    ...
    if( assigned(context) and  (device_array[device - 1].handle <> 0) ) then
    begin
        DeviceIoControl(device_array[device - 1].handle, IOCTL_SET_PRECEDENCE, @precedence, sizeof(InterceptionPrecedence), nil, 0, @bytes_returned, nil);
    end;
end;
...
begin
    ...
    if( assigned(context) and  (device_array[device - 1].handle <> 0) ) then
    begin
        DeviceIoControl(device_array[device - 1].handle, IOCTL_GET_FILTER, nil, 0, @result, sizeof(InterceptionFilter), @bytes_returned, nil);
    end;
end;
    ...
    begin
        ...
        begin
            if((device_array[i].handle <> 0) and interception_predicate(i + 1)  ) then
                DeviceIoControl(device_array[i].handle, IOCTL_SET_FILTER, @filter, sizeof(InterceptionFilter), nil, 0, @bytes_returned, nil);
        end;
...
function interception_wait_with_timeout(context: InterceptionContext; milliseconds: NativeUInt): InterceptionDevice;
var device_array : InterceptionDeviceArray ;
var wait_handles : array [0..pred(INTERCEPTION_MAX_DEVICE)] of HANDLE ;
var i, j, k : cardinal ;
begin
    Result := 0;
    if not assigned(context) then exit;
    ...
    if(k =  WAIT_FAILED ) or (k = WAIT_TIMEOUT) then exit;
...
function interception_send(context: InterceptionContext; device: InterceptionDevice; const stroke: PInterceptionStroke; nstroke: Cardinal): Integer;
 ...
 procedure sendKeystrokes;
    var rawstrokes : PKEYBOARD_INPUT_DATA;
    var rawstrokes_size : integer;
    var i : integer;
    var key_stroke : PInterceptionKeyStroke;
  begin
        rawstrokes_size := nstroke * sizeof(KEYBOARD_INPUT_DATA);

        rawstrokes := PKEYBOARD_INPUT_DATA (GetMem(rawstrokes_size));

        if not assigned(rawstrokes) then exit;
        ...
        DeviceIoControl(device_array[device - 1].handle, IOCTL_WRITE, @rawstrokes[0],rawstrokes_size, nil, 0, @result, nil);
  ...
  procedure sendMouse;
    var rawstrokes : PMOUSE_INPUT_DATA;
    var rawstrokes_size : integer;
    var i : integer;
    var mouse_stroke : PInterceptionMouseStroke;
  ...
  begin
        ...
        DeviceIoControl(device_array[device - 1].handle,IOCTL_WRITE,rawstrokes, rawstrokes_size, nil, 0, @result, nil);
...
var device_array : InterceptionDeviceArray ;
  ...
  begin
     rawstrokes_size := nstroke * sizeof(KEYBOARD_INPUT_DATA);
        ...
        DeviceIoControl(device_array[device - 1].handle, IOCTL_READ, nil, 0, rawstrokes,rawstrokes_size, @result, nil);
...
begin
    ...
    DeviceIoControl(device_array[device - 1].handle, IOCTL_GET_HARDWARE_ID, nil, 0, hardware_id_buffer, buffer_size, @result, nil);
...
begin
    result := false;
    if interception_is_keyboard(device) then exit;
    if interception_is_mouse(device) then exit;
    result := true;
end;

function interception_is_keyboard(device: InterceptionDevice): LongBool;
begin
     result := (device >= INTERCEPTION_KEYBOARD(0)) and  ( device <= INTERCEPTION_KEYBOARD(INTERCEPTION_MAX_KEYBOARD - 1));
end;

function interception_is_mouse(device: InterceptionDevice): LongBool;
begin
   result := (device >= INTERCEPTION_MOUSE(0) ) and (device <= INTERCEPTION_MOUSE(INTERCEPTION_MAX_MOUSE - 1));
end;
...
const offset = INTERCEPTION_MAX_KEYBOARD + 1;
begin
  Result :=  index + offset;
end;
